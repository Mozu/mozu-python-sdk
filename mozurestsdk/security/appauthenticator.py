...
class AppAuthenticator(object):
	...
	def __init__(self,applicationKey, sharedSecret, authUrl):
	...
	def authenticate(self):
		...
		auth_response = util.http_call(auth_url,"POST", data=json.dumps(auth_request), headers=headers, verify=False);
	...
	def refreshAuth(self):
		...
		auth_response = util.http_call(auth_url,"PUT", data=json.dumps(auth_request), headers=headers, verify=False);
		self.auth = auth_response.json();
